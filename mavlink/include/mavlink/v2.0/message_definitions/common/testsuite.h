...
static void mavlink_test_peer_position(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_PEER_POSITION >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_peer_position_t packet_in = {
        963497464,963497672,963497880,963498088,963498296,18275,199,10
    };
    mavlink_peer_position_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.time_boot_ms = packet_in.time_boot_ms;
        packet1.lat = packet_in.lat;
        packet1.lon = packet_in.lon;
        ...
        packet1.relative_alt = packet_in.relative_alt;
        packet1.hdg = packet_in.hdg;
        packet1.peer_id = packet_in.peer_id;
        packet1.static_pos = packet_in.static_pos;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_PEER_POSITION_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_PEER_POSITION_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_peer_position_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_peer_position_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_peer_position_pack(system_id, component_id, &msg , packet1.time_boot_ms , packet1.peer_id , packet1.static_pos , packet1.lat , packet1.lon , packet1.alt , packet1.relative_alt , packet1.hdg );
    mavlink_msg_peer_position_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_peer_position_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.time_boot_ms , packet1.peer_id , packet1.static_pos , packet1.lat , packet1.lon , packet1.alt , packet1.relative_alt , packet1.hdg );
    mavlink_msg_peer_position_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_peer_position_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_peer_position_send(MAVLINK_COMM_1 , packet1.time_boot_ms , packet1.peer_id , packet1.static_pos , packet1.lat , packet1.lon , packet1.alt , packet1.relative_alt , packet1.hdg );
    mavlink_msg_peer_position_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("PEER_POSITION") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_PEER_POSITION) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_peer_position(system_id, component_id, last_msg);
